...
import "../../App.css";
import { useOrdermateInvoice } from "../../hooks/useOrdermateInvoice";
import OrderToInvoiceList from "./OrderToInvoiceList";

const OrderToInvoice = () => {
  const { data: invoiceData, refetch } = useOrdermateInvoice();
  ...
  const filteredOrders = useMemo(() => {
    ...
    return invoiceData.filter((order) => {
      ...
      if (sortOrder === "desc") return (b.total || 0) - (a.total || 0);
      return 0;
    });
    ...
    <div className="font ml-14 p-6 min-h-screen bg-gray-50 flex flex-col flex-1">
      ...
      <div className="w-full mb-8">
        <div className="flex justify-between items-end">
          <div>
            <h1 className="text-3xl font-extrabold text-gray-900 tracking-tight">Order To Invoice</h1>
            <p className="text-gray-500 mt-2 font-medium">
              Showing <span className="text-blue-600 font-bold">{filterType === 'today' ? "Today's" : "All"}</span> ordermate orders
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden flex flex-col">
        <div className="px-8 py-6 border-b border-gray-50 flex justify-between items-center">
          <h2 className="text-xl font-bold text-gray-800">Ordermate Invoices</h2>
        ...
        <div className="flex-1 overflow-auto">
          {filteredOrders.length === 0 ? (
            ...
            <OrderToInvoiceList data={currentOrders} refetch={refetch} />
          )}
        </div>
